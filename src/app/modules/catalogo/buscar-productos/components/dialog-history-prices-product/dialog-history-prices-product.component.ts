...
export class DialogHistoryPricesProductComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.isLoading = true;
   this.subscription =  this.methodsHttp.methodGet<{prices: HistoryPriceProduct[]}>(`catalogs/products/${this.data.product.id}/history-prices`).subscribe((res) => {
      if (res?.success) {
