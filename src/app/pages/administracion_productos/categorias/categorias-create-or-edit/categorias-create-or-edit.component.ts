     ...
     this.act_router.data.subscribe(res=>{
      ...
      if(res.isEdit){
        ...
        this.s_categories.edit(this.id).subscribe(response=>{
          ...
          this.formCreateOrEdit.controls['name'].setValue(response.category.name);
          this.formCreateOrEdit.controls['sort_name'].setValue(response.category.sort_name);
  ...
  sendData():void{
    if(this.formCreateOrEdit.valid){
      ...
      if(!this.id){
        ...
        console.log(this.formCreateOrEdit.value);
        this.s_categories.update(this.id,this.formCreateOrEdit.value).subscribe(
          res=>{
            console.log(res);
            if (res.hasOwnProperty('success') && res.success) {
