...
export class CreateOrEditAddressClientComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<CreateOrEditAddressClientComponent>,
    @Inject(MAT_DIALOG_DATA) public data: { isObligate: boolean, client: IClientOrder, address_id?: number}, private s_standard: StandartSearchService) {
  }
  ...
  edit(): void {
    this.isLoading = true;
    this.s_standard.methodGet<IClientAddressOrder>('system-orders/clients/' + this.data.client.id + '/addresses/' + this.data.address_id + '/edit').subscribe(res => {
      console.log(res);
  ...
  saveInServer(): void {
    this.isLoading = true;
    let url = 'system-orders/clients/' + this.data.client.id + '/addresses';
    ...
    if (this.status === 'edit' && this.data.address_id) {
      ...
      observable = this.s_standard.store(url, this.form.value);
    }
